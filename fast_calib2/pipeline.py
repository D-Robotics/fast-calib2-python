...
def extract_image_centers(image_path):
    ...
    # ArUco API 容错: OpenCV 4.7+ 用 ArUcoDetector 类; 旧版用 getPredefinedDictionary + detectMarkers.
    ...
    elif hasattr(cv2.aruco, "Detector"):
    ...
    # 板姿态: 优先 estimatePoseBoard (旧 API), 失败/不存在则 solvePnP (4.7+ 移除了 estimatePose*).
    # _BOARD_CORNERS[i] 是第 i 个 marker 的 4 个 3D 角点(板系), BOARD_IDS[i] 是其 id.
    ids_flat = np.asarray(ids).flatten()
    obj_pts = []; img_pts = []
    for i, mid in enumerate(ids_flat):
        if mid in BOARD_IDS:
            slot = BOARD_IDS.index(mid)
            obj_pts.extend(_BOARD_CORNERS[slot])
            img_pts.extend(corners[i].reshape(-1, 2))
    ok = 0; rvec = None; tvec = None
    if len(obj_pts) >= 4:
        try:
            ok, rvec, tvec = cv2.solvePnP(np.array(obj_pts, np.float64),
                                          np.array(img_pts, np.float64),
                                          CAM_MTX, CAM_DIST, flags=cv2.SOLVEPNP_ITERATIVE)
        except Exception:
            ok = 0
    if not ok:
        print(f"  [IMG] 板姿态 solvePnP 失败 (匹配角点 {len(obj_pts)})"); return None, None, None
...
def main(argv=None):
    ...
    if args.multi:
        ...
        if len(sel) != 3:
            print(f"指定的3场景里只有 {len(sel)} 个成功"); return results
        # 直接用这3个
